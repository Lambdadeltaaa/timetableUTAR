...
- OUTPUT: 
    parsing success: object timetableData, contains relevant timetable infos which is further explained edit-timetable.js
...
export function parseHTML(HTMLInput) {
    ...
    if (days.length === 0) {
        return { error: "No days in timetable found. The timetable's code may have been changed."};
    }
    let units = timetable.getElementsByClassName(CLASSNAMES.unitCell);
    if (units.length === 0) {
        return { error: "No classes/units found in timetable. The timetable's code may have changed, or you have no classes at all."}
    }
